...
def buildWGMmodel(WGMr,WGr,GAP,fsrc,df,resolution,nfreq):
    ...
    Film = mp.Cylinder(radius=WGMr+Fthick, material=mp.Medium(index=2.50))

    Mask1 = mp.Block(mp.Vector3(50,85,mp.inf),
            center=mp.Vector3(x=XPos,y=0),
            material=mp.Medium(index=1.0)
            )

    Mask2 = mp.Block(mp.Vector3(50,85,mp.inf),
            center=mp.Vector3(x=-XPos,y=0),
            material=mp.Medium(index=1.0)
            )

    Mask3 = mp.Block(mp.Vector3(80,20,mp.inf),
            center=mp.Vector3(x=0,y=-40),
            material=mp.Medium(index=1.0)
            )


    geometry = [Film,Mask1,Mask2,Mask3,WaveGuide,Circle]
